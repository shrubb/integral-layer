...
local _, parent = torch.class('IntegralNoNorm', 'nn.Module')
...
do
    ...
    function IntegralNoNorm:__init(nWindows, h, w)
        parent.__init(self)
    ...
    function IntegralNoNorm:type(type, tensorCache)
        if not type then
            return self._type
        end
    ...
    -- overload
    function IntegralNoNorm:write(file)
    ...
    -- overload
    function IntegralNoNorm:read(file)
    ...
    function IntegralNoNorm:parameters()
    ...
    function IntegralNoNorm:updateGradInput(input, gradOutput)
        if self.gradInput then
            ...
            self.backpropHelper = self.backpropHelper or IntegralNoNorm(1, self.h, self.w):type(self._type)
    ...
    function IntegralNoNorm:zeroGradParameters()
        self.gradXMin:zero()
        self.gradYMin:zero()
        self.gradXMax:zero()
        self.gradYMax:zero()
    ...
    function IntegralNoNorm:updateParameters(lr)
