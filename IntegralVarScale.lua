...
local _, parent = torch.class('IntegralVarScale', 'nn.Module')
...
void updateGradInputVarScaleCuda(
    ...
    float *xMin, float *xMax, float *yMin, float *yMax,
    int strideH, int strideW, const float *scaleData);
...
if pcall(require, 'cutorch') then
    ...
    CUDA_lib_varscale._initCublasHandleVarScale();
end
...
do
    ...
    function IntegralVarScale:type(type, tensorCache)
        if not type then
            return self._type
        end
    ...
    function IntegralVarScale:resetSingleWindow(idx)
    ...
    function IntegralVarScale:parameters()
    ...
    function updateOutputGPU(self, input)
        ...
        if self.normalize then
            ...
            -- for C function safety
            assert(self.outputOnes:stride(3) == self.wOut)
            ...
            for batchIdx = 1,batchSize do
                ...
                if self.exact then
                    if self.replicate then
                        forwardCFunction = CUDA_lib_varscale.forwardNoNormReplicateFracVarScaleCuda
                    else
                        error('NYI')
                        forwardCFunction = C_lib.forwardNoNormFrac
                    end
        ...
        do
            ...
            -- for C function safety
            assert(self.outputNonNorm:stride(3) == self.wOut)
            assert(self.outputNonNorm:stride(2) == self.wOut*self.hOut) 
    ...
    function IntegralVarScale:updateGradInput(input, gradOutput)
    ...
    function IntegralVarScale:zeroGradParameters()
        self.gradXMin:zero()
        self.gradYMin:zero()
        self.gradXMax:zero()
        self.gradYMax:zero()
    ...
    function IntegralVarScale:updateParameters(lr)
