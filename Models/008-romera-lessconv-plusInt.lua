...
local function btInt(inChannels, btChannels, nWindows, h, w, dropoutProb)
    dropoutProb = dropoutProb or 0
    ...
    local mainBranch = nn.Sequential()
        ...
        :add(SpatialBatchNormalization(btChannels*nWindows + btChannels))
        :add(ReLU(true))

        :add(SpatialConvolution(btChannels*nWindows + btChannels, inChannels, 1,1, 1,1, 0,0))
        :add(SpatialBatchNormalization(inChannels))
        :add(dropoutProb ~= 0 and nn.SpatialDropout(dropoutProb) or nn.Identity())
...
model:add(btInt(32, 16, 20, h/4, w/4, 0.03))
model:add(btInt(32, 16, 20, h/4, w/4, 0.03))
model:add(btInt(32, 16, 20, h/4, w/4, 0.03))
model:add(btInt(32, 16, 20, h/4, w/4, 0.03))
...
model:add(btInt(32, 16, 16, h/4, w/4))
